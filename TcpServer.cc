...
/*
角色	                   对应 muduo 组件	                              核心作用
mainLoop	            主线程的EventLoop	                 主 Reactor，只负责监听端口、接收新连接
TcpServer	            服务器顶层管理类	                  新连接的最终调度者
Acceptor	            连接接收器	                          封装监听 socket，负责accept客户端连接
acceptChannel	        监听 fd 的事件通道	                  把listenfd注册到Poller，作为事件载体
Poller	                多路复用器（epoll）	                  内核事件监听，检测listenfd是否有新连接
